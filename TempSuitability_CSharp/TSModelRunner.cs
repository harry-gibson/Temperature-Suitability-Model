...
namespace TempSuitability_CSharp
{
    class TSModelRunner
    {
        ...
        private readonly IFilenameDateParser _modisParser;
        private readonly IFilenameDateParser _humParser;
        ...
        public TSModelRunner(IFilenameDateParser _parser, string maskPath, string dayPath, string nightPath, string outDir, int maskValidValue)
        {
            _modisParser = _parser;
            //System.Console.WriteLine("Looking for files in " + m_FileWildCard);
            //var d = new System.Diagnostics.DefaultTraceListener();
            _maxReader = new TiffCubeReader(dayPath, _modisParser);
            var nMax = _maxReader.Filenames.Count;
            System.Console.WriteLine("Looking for LST Day files in " + dayPath + 
                " - found "+ nMax.ToString());
            _minReader = new TiffCubeReader(nightPath, _modisParser);
            ...
            :this(modisParser, maskPath, dayPath, nightPath, outDir, maskValidValue)
        {
            ...
            _humReader = new TiffCubeReader(humPath, _humParser);
